...
public class DefaultSimpleExtractor implements TextExtractionStrategy {
     ...
     */
    private final ArrayList<TextRenderInfo> blockTextInfo = new ArrayList<>();
    /**
     * List of extracted blocks.
     */
    private final ArrayList<Block> extractedBlocks = new ArrayList<>();
    /**
     ...
     * We estimate this direction by taking the direction of most of the block.
     */
    ...
    public void renderText(TextRenderInfo textRenderInfo) {
        ...
        blockTextInfo.add(textRenderInfo);
    }

    private void cloneTextRenderInfoField(TextRenderInfo textRenderInfo) {
        Class<? extends TextRenderInfo> textRenderInfoClass = textRenderInfo.getClass();
    ...
    private Block createBlock(double xMin, double xMax, double yMin, double yMax) {
        ...
        for (TextRenderInfo render : blockTextInfo) {
    ...
    private void registerBlockDirection(Direction direction) {
        switch (direction) {
            case LEFT -> nBlockFacingTop++;
            case RIGHT -> nBlockFacingBottom++;
            case TOP -> nBlockFacingLeft++;
            case BOTTOM -> nBlockFacingRight++;
        }
    }
     ...
     * @param lastAscent  one character's upside center
     * @param lastDescent one character's downside center
    ...
    private double getMinAscent(int axisIndex) {
        double minAscent = blockTextInfo.get(0).getAscentLine().getStartPoint().get(axisIndex);

        for (TextRenderInfo textRenderInfo : blockTextInfo) {
    ...
    private double getMaxAscent(int axisIndex) {
        double maxAscent = blockTextInfo.get(0).getAscentLine().getStartPoint().get(axisIndex);
    ...
    private double getMinDescent(int axisIndex) {
        double minDescent = blockTextInfo.get(0).getDescentLine().getStartPoint().get(axisIndex);
    ...
    private double getMaxDescent(int axisIndex) {
        double maxDescent = blockTextInfo.get(0).getDescentLine().getStartPoint().get(axisIndex);
        ...
        if (nBlockFacingTop + nBlockFacingBottom > nBlockFacingLeft + nBlockFacingRight) {
            ...
            if (nBlockFacingTop <= nBlockFacingBottom) {
                ...
                for (Block block : extractedBlocks) {
                    ...
                    switch (block.getBlockOrientation()) {
                        case TOP -> block.setBlockOrientation(Direction.BOTTOM);
                        case BOTTOM -> block.setBlockOrientation(Direction.TOP);
                        case LEFT -> block.setBlockOrientation(Direction.RIGHT);
                        case RIGHT -> block.setBlockOrientation(Direction.LEFT);
                    }
            ...
            if (nBlockFacingLeft > nBlockFacingRight) {
                ...
                for (Block block : extractedBlocks) {
                    // Transform direction
                    switch (block.getBlockOrientation()) {
                        case TOP -> block.setBlockOrientation(Direction.LEFT);
                        case BOTTOM -> block.setBlockOrientation(Direction.RIGHT);
                        case LEFT -> block.setBlockOrientation(Direction.BOTTOM);
                        case RIGHT -> block.setBlockOrientation(Direction.TOP);
                    }
                ...
                for (Block block : extractedBlocks) {
                    switch (block.getBlockOrientation()) {
                        case TOP -> block.setBlockOrientation(Direction.RIGHT);
                        case BOTTOM -> block.setBlockOrientation(Direction.LEFT);
                        case LEFT -> block.setBlockOrientation(Direction.TOP);
                        case RIGHT -> block.setBlockOrientation(Direction.BOTTOM);
                    }
